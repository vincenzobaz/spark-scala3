...
class UdfSpec extends munit.FunSuite:
  ...
  test("select udf returning an option in case class") {
    udf((name: String, x: Int) =>
      DataWithOptX(Option.when(name.length > 1)(name), 2 * x)
    )
      .register("dataoptx")
    val input =
      Seq(DataWithPos("", 0, 0, 0), DataWithPos("something", 1, 2, 3))
    val df = input.toDF()
    df.createOrReplaceTempView("data")
    val res = spark
    ...
    val cmp = input.map { case DataWithPos(name, x, y, z) =>
      ContainsDataOpt(DataWithOptX(Option.when(name.length > 1)(name), 2 * x))
    }
